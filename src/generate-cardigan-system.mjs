...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'cardigan', 'Cardigan', and 'CARDIGAN'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.cardigan/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.cardigan/g),
   ...
   * Rename files.
   *
   * Rename files that had cardigan in their name, such as
   * css/cardigan.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*cardigan*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('cardigan', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-cardigan-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Cardigan's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace cardigan name mentions in files.
      generator.replaceFileContents();
      // Rename files that had cardigan in their name.
