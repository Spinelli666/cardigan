...
async function handleExecuteTradeTransfer(data) {
  const { initiatorId, targetId, initiatorItems, targetItems, initiatorGold, targetGold } = data;
  ...
  try {
    ...
    for (const tradeItem of initiatorItems) {
      ...
      if (existingItem) {
        ...
        // Wait a tick to ensure the actor's items collection is updated
    ...
    if (initiatorGold > 0) {
      const initiatorCurrentGold = initiator.system.moedas?.po || 0;
      const targetCurrentGold = target.system.moedas?.po || 0;
      
      await initiator.update({
        'system.moedas.po': initiatorCurrentGold - initiatorGold
      });
      
      await target.update({
        'system.moedas.po': targetCurrentGold + initiatorGold
      });
    }
    ...
    if (targetGold > 0) {
      ...
      await target.update({
        'system.moedas.po': targetCurrentGold - targetGold
      });
      
      await initiator.update({
        'system.moedas.po': initiatorCurrentGold + targetGold
      });
    }
    ...
    handleTradeComplete({ tradeId: data.tradeId });
