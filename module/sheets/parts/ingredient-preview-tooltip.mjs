 ...
 * item-ingredient's icon in the backpack list, and posts the same content as a chat message.
 * Mirrors CommonPreviewTooltip (item-comum), reusing the same
 * templates/tooltips/item-preview-tooltip.hbs — item-ingredient has no armor/movement/critical/
 ...
 */
export class IngredientPreviewTooltip {
  /**
  ...
  static async attach(html, actor) {
    ...
    for (const icon of icons) {
      ...
      if (item?.type !== 'item-ingredient') continue;

      try {
        icon.dataset.tooltipHtml = await this._buildTooltipHtml(item);
      } catch (error) {
        console.error('[CARDIGAN] Error building ingredient preview tooltip:', error);
      }
    }
  }
